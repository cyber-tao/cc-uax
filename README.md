...
- **Rich value decoding**
  ...
  | Runtime structs | `InstancedStruct` / `InstancedStructContainer`, `StateTreeInstanceData`, `PerQualityLevelInt` / `Float`, delegates (`Delegate` / `MulticastInline` / `MulticastSparse`), `EdGraphPinType` |
  | Gameplay, PCG & FX | `GameplayTagContainer`, `GameplayEffectVersion`, `Spline`, `AlphaBlend`, StateTree references/links, PCG selectors + point arrays, Niagara core (`NiagaraVariable` / `NiagaraVariableBase` / `NiagaraVariableWithOffset` / `NiagaraTypeDefinition`) |
...
- ✅ **Validated** on real UE5.7 editor `.uasset` / `.umap` files — failed = 0, diagnostics = 0, `@unparsed` = 0. Point the validation script at any UE5 project: `.\scripts\validate-real-assets.ps1 -ContentDir <your-project>/Content` (PowerShell) or `CC_UAX_CONTENT_DIR=<your-project>/Content ./scripts/validate-real-assets.sh` (Bash); optionally pin a reverse-reference fixture with `-ReverseRefInput <asset.uasset> -ExpectedReferencer /Game/.../Referencer`.
- ❌ Cooked packages (unversioned / package compression) and UE4 legacy formats are **not** supported.
- 🔧 Native-binary structs used by the current UE5.7 validation set — including StateTree, PCG, InstancedStruct containers, Niagara, GPU binding, groom dataflow, skeletal-mesh sampling, and cloth LOD payloads — are decoded structurally; registry-dependent future custom payloads still use an alignment-preserving typed hex/opaque preview.
