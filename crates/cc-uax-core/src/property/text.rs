...
pub(crate) fn parse_text(r: &mut Reader, ctx: &ParseCtx, depth: usize) -> Result<Value> {
    if depth > 32 {
    ...
    match history_type {
        ...
        1 => {
            ...
            let format = parse_text(r, ctx, depth + 1)?;
            ...
            for _ in 0..count {
                ...
                let value = parse_format_argument(r, ctx, depth + 1)?;
        ...
        2 => {
            ...
            let mut arguments = Vec::with_capacity(count as usize);
            for _ in 0..count {
                arguments.push(parse_format_argument(r, ctx, depth + 1)?);
            }
            Ok(json!({
        ...
        3 => {
            ...
            for _ in 0..count {
                let name = r.read_fstring()?;
                let value = parse_format_argument_data(r, ctx, depth + 1)?;
                arguments.push(json!({ "name": name, "value": value }));
            }
            Ok(json!({
        ...
        4 => parse_number_format_history(r, ctx, "AsNumber", flags, false, depth),
        5 => parse_number_format_history(r, ctx, "AsPercent", flags, false, depth),
        6 => parse_number_format_history(r, ctx, "AsCurrency", flags, true, depth),
        7 => {
        ...
        10 => {
            ...
            let source = parse_text(r, ctx, depth + 1)?;
        ...
        11 => {
            ...
            let table_id = ctx.names.resolve_raw(r.read_raw_name()?);
            let key = r.read_fstring()?;
            Ok(json!({
        ...
        12 => {
            ...
            let type_id = ctx.names.resolve_raw(r.read_raw_name()?);
            let mut o = Map::new();
...
fn parse_format_argument(r: &mut Reader, ctx: &ParseCtx, depth: usize) -> Result<Value> {
    let arg_type = r.read_i8()?;
    Ok(match arg_type {
        0 => json!(r.read_i64()?),           // Int
        1 | 5 => json!(r.read_u64()?),       // UInt / Gender (Gender is stored as a UInt)
        2 => json!(r.read_f32()? as f64),    // Float
        3 => json!(r.read_f64()?),           // Double
        4 => parse_text(r, ctx, depth + 1)?, // Text
...
fn parse_number_format_history(
    ...
    let source_value = parse_format_argument(r, ctx, depth + 1)?;
...
fn parse_format_argument_data(r: &mut Reader, ctx: &ParseCtx, depth: usize) -> Result<Value> {
