...
pub(crate) fn framework_pin_version(summary: &PackageFileSummary) -> Option<i32> {
    summary.custom_version(custom::FRAMEWORK_OBJECT_VERSION)
}

pub(crate) fn is_supported_framework_pin_version(version: i32) -> bool {
    version <= custom::FRAMEWORK_LATEST
}
...
pub(crate) fn parse_user_defined_pins_report(
    ...
    let parsed = (|| -> Result<Vec<UserDefinedPin>> {
        ...
        for _ in 0..count {
            let name = if framework_version >= custom::FRAMEWORK_PINS_STORE_FNAME {
...
fn parse_pins_inner(
    ...
    for _ in 0..count {
        ...
        let pin = parse_pin(r, end, ctx, vc)?;
        pins.push(pin);
        if r.pos() > end {
...
fn parse_pin(r: &mut Reader, end: u64, ctx: &ParseCtx, vc: &PinSerCtx) -> Result<Pin> {
    ...
    let default_text = parse_text(r, ctx, end, 0)?;
