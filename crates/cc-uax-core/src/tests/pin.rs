...
fn logic_graphs_group_by_outer_and_never_emit_cross_graph_edges() {
    ...
    data_input.friendly_name = Some(crate::structured_value::json!("Amount"));
    ...
    data_input.default_text = crate::structured_value::json!({ "source": "Forty Two" });
    // An all-false editor-flags block must collapse to no `editor_flags` key.
    data_input.editor_flags = Some(crate::pin::PinEditorFlags {
        hidden: false,
        not_connectable: false,
        default_value_read_only: false,
        default_value_ignored: false,
        advanced_view: false,
        orphaned_pin: false,
    });
...
fn graph_pin(pin_id: Guid, direction: u8, category: &str, links: Vec<(i32, Guid)>) -> Pin {
    Pin {
        ...
        default_object: 0,
        default_text: crate::DecodedValue::Null,
        linked_to: links
            .into_iter()
