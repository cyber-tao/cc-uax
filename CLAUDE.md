...
- `package.rs` only attempts pin parsing for graph-node classes (`is_graph_node_class`: `K2Node*` / `EdGraphNode*` / `NiagaraNode*` / `NiagaraOverviewNode` / `*GraphNode*`; the `*Binding*` exclusion for helpers such as `AnimGraphNodeBinding_Base` only applies to the fuzzy `*GraphNode*` match), then resolves each `LinkedTo` reference to a readable `{ node, pin }` via a cross-node `(node_index, pin_guid) → name` map.
