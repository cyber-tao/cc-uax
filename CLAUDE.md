...
UE5.0–5.8 source is the serialization authority. The `FileVersionUE5` each release writes, computed from that release branch's `ObjectVersion.h`: 5.0 = 1004, 5.1 = 1008, 5.2 and 5.3 = 1009, 5.4 = 1012, 5.5 = 1013, 5.6 = 1017, 5.7 and 5.8 = 1018. Values 1000–1003 and 1005–1007 come from pre-release 5.0/5.1 mainline builds and do occur in real projects. The parser targets versioned, uncooked UE5.0–5.8 editor packages (`FileVersionUE5` 1000–1018). `VERIFIED_FILE_VERSION_FLOOR` currently matches `SUPPORTED_FILE_VERSION_FLOOR` (UE5.0 / 1000), so accepted packages may be `status=complete` when their evidence is complete.
...
Some classes never write a tagged block at all: `URigHierarchy`, `URigVM` and `ULandscapeHeightmapTextureEdgeFixup` do not call `Super::Serialize`. From `FileVersionUE5` 1010 the export table says so directly — `UObject::SerializeScriptProperties` is what marks `ScriptSerializationStart/EndOffset`, a written block is never shorter than its `None` terminator, so a zero range means no block (`SavePackage2.cpp` asserts it; `LinkerLoad.cpp` skips tagged loading on it) — and older packages use a source-verified class list (`is_native_only_payload_class`). Such an export is `property_status = native_only` with one `class_payload` region; running the tag loop on it fabricated properties out of whatever bytes resolved to a name. Conversely a class that writes data *before* its block (`UAssetImportData`'s JSON source-file list) needs a prefix decoder, or the tag loop starts in the middle of that data on packages without a declared range.

Byte conservation is computed from the spans each decoder claims, not from a high-water mark: bytes between two claims are a `decoder_gap` region counted as unattributed, and bytes two decoders both claim are `unclassified_bytes`. A diagnostic raised inside a nested tagged block reaches the report through `ParseCtx::nested_diagnostics`, re-rooted under the property that holds it, and any opaque property value downgrades `tagged_properties`; a value nobody could decode must never leave the asset `complete`.
...
- **Kismet bytecode** — `UStruct::Serialize` (`SuperStruct`, `Children`, `ChildProperties`, the two script sizes), `UFunction::Serialize` and `UClass::Serialize` are decoded as structured fields, and the `Script` stream is disassembled per `UObject/ScriptSerialization.inl`. Two sizes bracket that stream and they are different numbers: `BytecodeBufferSize` is the in-memory buffer the VM executes (a pointer is 8 bytes, an `FScriptName` is 12) and `SerializedScriptSize` is the on-disk length (the same pointer is a 4-byte `FPackageIndex`, the same name an 8-byte linker `FName`). Agreeing with only one of them is not evidence the walk is right, so both are checked. An unknown opcode has an unknown payload and must fail rather than be skipped.
- **Remaining compiled gaps** — `niagara_compiled`, `rig_vm_bytecode` and `rig_hierarchy` are still the same shape of gap: the source-level graph decodes, the compiled form does not, and each must be a named capability rather than an anonymous export tail.
...
`cc-uax-project` discovers either a project directory/`.uproject` or a `Content` directory and scans mapped assets once. Passing an explicit `.uproject` file selects that file even when sibling platform `.uproject` files share the same Content tree. A directory or Content path with more than one `.uproject` does not fail: which Content tree to scan was never in doubt, only which descriptor supplies entry points, so the scan proceeds with `project_file` unresolved, reads the shared `Config/`, and reports the candidates as a `Config` diagnostic. Platform variants sharing one Content tree are a normal layout, and failing there made those projects unscannable by their Content path.
...
Strict mode is the default, and `ScanFailureStage::is_hard` decides what it fails on. A mapped read/index/parse failure, or a mount/cache error, returns the partial index as a structured error and causes a non-zero CLI exit. `Discovery`, `Ownership` and `Config` failures are soft: an unlistable directory entry, or a World Partition external package whose owning map was renamed away, is real evidence and stays in `failures`, but every asset the scan could reach is still indexed, so it must not abort the run. Inherent partial or unsupported evidence (for example known-opaque compiled RigVM bytecode, or a package version outside the supported range) keeps a truthful non-complete `status` but does not by itself fail the process. `--allow-partial` downgrades a hard scan failure to a zero exit; it must not change report truth.
